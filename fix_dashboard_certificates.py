...
file_path = r"m:\my projects\ACADEMIC OS\academic_os\templates\dashboard.html"
    ...
    <div class="col-12">
        <div class="card border-0 bg-primary text-white shadow-lg overflow-hidden position-relative">
            <div class="card-body p-4 p-md-5 position-relative" style="z-index: 2;">
                <h1 class="fw-bold mb-2 display-6 display-md-5">{% trans "Welcome back" %}, {{ user.first_name|default:user.username }}! 👋</h1>
                {% if is_admin_view %}
                <p class="lead opacity-75 mb-0 fs-6 fs-md-5">{% trans "Overview of the entire academic system." %}</p>
                {% else %}
                <p class="lead opacity-75 mb-0 fs-6 fs-md-5">{% trans "Here's what's happening in your academic workspace today." %}</p>
                {% endif %}
            </div>
            <!-- Decorative Circle -->
                ...
                <div class="d-flex align-items-center mb-3">
                    ...
                    <div>
                        <h6 class="text-muted text-uppercase mb-1 small fw-bold">
                            {% if is_admin_view %}{% trans "Avg System KPI" %}{% else %}{% trans "My KPI Score" %}{% endif %}
                        </h6>
        ...
        <div class="card h-100 border-0 shadow-sm hover-elevate">
            <div class="card-body">
                <div class="d-flex align-items-center mb-3">
                    ...
                    <div>
                        <h6 class="text-muted text-uppercase mb-1 small fw-bold">
                            {% if is_admin_view %}{% trans "Completed Tasks (All)" %}{% else %}{% trans "Completed Tasks" %}{% endif %}
                        </h6>
                        <h2 class="mb-0 text-dark fw-bold">{{ completed_tasks_count }}</h2>
                    </div>
                </div>
                <small class="text-success fw-medium"><i class="bi bi-arrow-up-short"></i> {% trans "All time" %}</small>
            </div>
        </div>
    </div>
</div>
    ...
    <div class="col-md-8">
        <div class="card h-100 border-0 shadow-sm">
            <div class="card-header bg-white py-3 d-flex justify-content-between align-items-center">
                <h5 class="card-title mb-0">{% if is_admin_view %}{% trans "System Performance Analytics" %}{% else %}{% trans "Performance Analytics" %}{% endif %}</h5>
            </div>
            <div class="card-body">
    ...
    <div class="col-md-4">
        <div class="card h-100 border-0 shadow-sm">
            <div class="card-header bg-white py-3">
                <h5 class="card-title mb-0">{% if is_admin_view %}{% trans "System Task Distribution" %}{% else %}{% trans "Task Distribution" %}{% endif %}</h5>
            </div>
...
<div class="row g-4">
    ...
    <div class="col-md-4">
        <div class="card h-100 border-0 shadow-sm">
            ...
            <div class="card-body p-0">
                ...
                {% else %}
                <p class="p-4 text-center text-muted mb-0">{% trans "No pending tasks." %}</p>
                {% endif %}
            </div>
        </div>
    </div>
    ...
    <div class="col-md-4">
        <div class="card h-100 border-0 shadow-sm">
            <div class="card-header bg-white py-3">
                <h5 class="card-title mb-0">{% if is_admin_view %}{% trans "All Observations" %}{% else %}{% trans "Recent Observations" %}{% endif %}</h5>
            </div>
            <div class="card-body p-0">
                ...
                {% else %}
                <p class="p-4 text-center text-muted mb-0">{% trans "No recent observations." %}</p>
                {% endif %}
                <div class="card-footer bg-white text-center py-2">
                    <a href="{% url 'observation_list' %}" class="text-decoration-none small">{% trans "View All" %}</a>
                </div>
            </div>
        </div>
    </div>
    ...
    <div class="col-md-4">
        <div class="card h-100 border-0 shadow-sm">
            <div class="card-header bg-white py-3">
                <h5 class="card-title mb-0">{% if is_admin_view %}{% trans "Latest Certificates" %}{% else %}{% trans "My Certificates" %}{% endif %}</h5>
            </div>
            <div class="card-body p-0">
                ...
                <ul class="list-group list-group-flush">
                    ...
                    <li class="list-group-item d-flex justify-content-between align-items-center px-4 py-3">
                        <div>
                            ...
                            {% if is_admin_view %}
                            <small class="text-muted d-block"><i class="bi bi-person me-1"></i>{{ cert.employee.get_full_name|default:cert.employee.username }}</small>
                            {% endif %}
                            <small class="text-muted">{{ cert.date_issued|date:"d.m.Y" }}</small>
                        </div>
                        {% if cert.file %}
                        <a href="{{ cert.file.url }}" class="btn btn-sm btn-outline-primary" download><i class="bi bi-download"></i></a>
                        {% endif %}
                    </li>
                    {% endfor %}
                </ul>
                {% else %}
                <p class="p-4 text-center text-muted mb-0">{% trans "No certificates found." %}</p>
                {% endif %}
            </div>
        </div>
    </div>
